  ...
  install(Vue, opts = {}) {
    ...
    function getSize(){
      ...
      Vue.prototype.$graphInit = async function(options){
        ...
        let highlightNodes = store.state.graph.highlightNodes
        let highlightLinks = store.state.graph.highlightLinks
        let hoverNode = store.state.graph.hoverNode
        ...
        // console.log(graph)
        store.commit ('graph/setGraph', graph)
      }

      function highligth(node){
        //console.log(node)
        return store.state.graph.search != null && store.state.graph.search.text.length > 0 && node.name.includes(store.state.graph.search.text)
      }
      // Vue.prototype.$onNodeSearch = async function (node, event){
      //   console.log(node, event)
      //     // if (event.ctrlKey || event.shiftKey || event.altKey) { // multi-selection
      //     //   store.state.graph.selectedNodes.has(node) ? store.state.graph.selectedNodes.delete(node) : store.state.graph.selectedNodes.add(node);
      //     // } else { // single-selection
      //     //   const untoggle = store.state.graph.selectedNodes.has(node) && store.state.graph.selectedNodes.size === 1;
      //     //   store.state.graph.selectedNodes.clear();
      //     //   !untoggle && store.state.graph.selectedNodes.add(node);
      //     // }
      //     store.state.graph.selectedNodes.add(node)
      //
      //     store.state.graph.graph.nodeColor(store.state.graph.graph.nodeColor()); // update color of selected nodes
      //   }
      ...
      async function onNodeClick(node){
        ...
        store.state.graph.graph.cameraPosition(
          pos, // new position
          node, // lookAt ({ x, y, z })
